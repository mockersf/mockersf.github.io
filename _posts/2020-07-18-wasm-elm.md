...
it's not an issue, as we call the Wasm function only on user input and a user will probably always be slowere than
loading the Wasm library, but if we were to add a command tto the `init` of our Elm application, it would fail.
...
type alias Model =
    ...
    , b : Int
    , res : Int
    , wasm_ready : Bool -- We keep track of whether the Wasm library is ready
    }
...
update msg model =
    case msg of
        WasmReady ready ->
            ...
            , Cmd.none -- Here, we could call our outgoing port that would trigger a Wasm function
            )
        [...]
